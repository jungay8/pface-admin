...
function qry(_curr,_limit){
    ...
    $eleForm.attr("action", CONTEXT_PATH + '/front/faceimagelibs');
    $("#pageNumKey").val(_curr);
    $("#pageSizeKey").val(_limit);
    $("#pagefaceusescenename").val($('#queryfaceusescenename').val());
    $eleForm.submit();
}
...
function delfaceappimagelib(_id, deviceId, libId){
    layer.confirm('将删除远程和本地的图片，请务必谨慎，不可恢复，您确定要删除吗?', {btn: ['确定', '取消'], title: "提示"}, function () {
        ...
        $.ajax({
            ...
            url: CONTEXT_PATH+'/front/appimagelibs/logic-delete',
            async: true,   //是否为异步请求
            cache: false,  //是否缓存结果
            data: {
                //id:ids.toString()
