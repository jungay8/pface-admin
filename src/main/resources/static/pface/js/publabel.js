...
function delLab(_id){
    layer.confirm('您确定要删除吗?', {btn: ['确定', '取消'], title: "提示"}, function () {
        ...
        $.ajax({
            type: "POST",
            url: CONTEXT_PATH+'/front/publishlab/logic-delete-batch',
            async: true,   //是否为异步请求
            cache: false,  //是否缓存结果
            data: {
                //id:ids.toString()
...
function qry(_curr,_limit){
    ...
    $eleForm.attr("action", CONTEXT_PATH + '/front/publab');
    ...
    layer.open({
        ...
        ,yes: function(){
            ...
            layui.form.on('submit(fromVerify)', function(data){
                ...
                $.ajax({
                    type: "POST",
                    url: CONTEXT_PATH+'/front/publishlab/createLabel',
                    async: true,   //是否为异步请求
                    cache: false,  //是否缓存结果
                    //data: {id:ids.toString()},
                    data:$("#publabForm").serialize(),
                    dataType:"json",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                        //'Content-Type': 'application/json;charset=UTF-8'
...
function editLabel(_id){
    ...
    $.ajax({
        type: "GET",
        url: CONTEXT_PATH+'/front/publishlab/get',
        async: true,   //是否为异步请求
        cache: false,  //是否缓存结果
        data: {id:_id},
    ...
    layer.open({
        ...
        ,yes: function(){
            ...
            layui.form.on('submit(fromVerify)', function(data){
                ...
                $.ajax({
                    type: "POST",
                    url: CONTEXT_PATH+'/front/publishlab/update',
                    async: true,   //是否为异步请求
                    cache: false,  //是否缓存结果
                    //data: {id:ids.toString()},
                    data:$("#publabForm").serialize(),
