...
function delLab(_id){
    layer.confirm('您确定要删除吗?', {btn: ['确定', '取消'], title: "提示"}, function () {
        ...
        $.ajax({
            type: "POST",
            url: CONTEXT_PATH+'/front/pricelab/logic-delete-batch',
            async: true,   //是否为异步请求
            cache: false,  //是否缓存结果
            data: {
                //id:ids.toString()
...
function qry(_curr,_limit){
    ...
    $eleForm.attr("action", CONTEXT_PATH + '/front/price');
    ...
    layer.open({
        ...
        ,yes: function(){
            ...
            layui.form.on('submit(fromVerify)', function(data){
                ...
                $.ajax({
                    type: "POST",
                    url: CONTEXT_PATH+'/front/pricelab/createLabel',
                    async: true,   //是否为异步请求
                    cache: false,  //是否缓存结果
...
function editLabel(_id){
    ...
    $.ajax({
        type: "GET",
        url: CONTEXT_PATH+'/front/pricelab/get',
        async: true,   //是否为异步请求
        cache: false,  //是否缓存结果
        data: {id:_id},
        //data:$("#priceForm").serialize(),
    ...
    layer.open({
        ...
        ,yes: function(){
            ...
            layui.form.on('submit(fromVerify)', function(data){
                ...
                $.ajax({
                    type: "POST",
                    url: CONTEXT_PATH+'/front/pricelab/update',
                    async: true,   //是否为异步请求
                    cache: false,  //是否缓存结果
                    //data: {id:ids.toString()},
                    data:$("#priceForm").serialize(),
