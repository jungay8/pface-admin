...
function qry(_curr,_limit){
    ...
    $eleForm.attr("action", CONTEXT_PATH + '/front/contentPause');
    $("#pageNumKey").val(_curr);
    $("#pageSizeKey").val(_limit);
    //媒体类型
    //jquery获取复选框值
   /* var chk_value =[];//定义一个数组
    $('input[name="tabelTime"]:checked').each(function(){//遍历每一个名字为interest的复选框，其中选中的执行函数
        chk_value.push($(this).val());//将选中的值添加到数组chk_value中
    });*/
    var t=$('input[type="radio"][name="tabelTime"]:checked').val();
    $("#tabelTime").val(t);
    //商品状态
    $("#times").val($('#LAY_chice_s').val());
